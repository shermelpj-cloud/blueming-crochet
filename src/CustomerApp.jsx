...
function PageShell({ children, onBack, title }) {
  return (
    <div className="min-h-screen" style={{ background: "#FFF9FB" }}>
      {onBack && (
        <div className="sticky top-0 z-20 flex items-center gap-2 px-4 py-3" style={{ background: "#FFF9FB", borderBottom: "1px solid #F4C0D1" }}>
          <button onClick={onBack} className="flex items-center justify-center rounded-full" style={{ width: 32, height: 32, background: "#FBEAF0" }}>
...
export default function BluemingCrochet() {
  ...
  if (page === "home") {
    return (
      <div style={{ background: "#FFF9FB", minHeight: "100vh" }} className="flex flex-col items-center">
        <div className="w-full max-w-md">
          <div className="text-center px-6 pt-12 pb-8" style={{ background: "linear-gradient(180deg,#F4C0D1 0%,#FBEAF0 100%)", borderBottomLeftRadius: 32, borderBottomRightRadius: 32 }}>
            <div className="mx-auto flex items-center justify-center rounded-full mb-3" style={{ width: 72, height: 72, background: "#fff" }}>
              <Flower2 size={34} color="#D4537E" />
            </div>
            <h1 className="font-medium" style={{ fontSize: 24, color: "#4B1528", letterSpacing: 0.5 }}>Blueming Crochet</h1>
          </div>
          <div className="px-6 py-8 text-center">
            <h2 className="font-medium mb-3" style={{ fontSize: 19, color: "#4B1528" }}>{home.title}</h2>
            <p className="text-sm leading-relaxed mb-6" style={{ color: "#72243E" }}>{home.description}</p>
            <button onClick={() => setPage("shop")} className="w-full py-3 rounded-full font-medium text-sm" style={{ background: "#D4537E", color: "#fff" }}>
              Shop now
            </button>
          </div>
          <div className="px-6 pb-6">
            <p className="text-xs font-medium mb-3" style={{ color: "#72243E" }}>Browse by category</p>
            <div className="grid grid-cols-4 gap-3">
              {CATEGORIES.map((c) => (
                <button key={c} onClick={() => { setActiveCategory(c); setPage("category"); }} className="flex flex-col items-center gap-1">
    ...
    return (
      <PageShell onBack={() => setPage("home")} title="Shop">
        <div className="max-w-md mx-auto">
          <div className="mx-4 mt-4 rounded-2xl p-5 text-center" style={{ background: "#FBEAF0" }}>
            <div style={{ fontSize: 26 }}>🧶</div>
            <h2 className="font-medium mt-1 mb-2" style={{ color: "#4B1528" }}>{shop.title}</h2>
            <p className="text-xs leading-relaxed" style={{ color: "#72243E" }}>{shop.description}</p>
          </div>
              ...
              return (
                <div key={cat} className="mb-7">
                  <div className="flex items-center justify-between mb-3">
                    <h3 className="font-medium text-sm" style={{ color: "#4B1528" }}>{ICONS[cat]} {cat}</h3>
      ...
      <PageShell onBack={() => setPage("shop")} title={activeCategory}>
        <div className="max-w-md mx-auto">
          <div className="mx-4 mt-4 rounded-2xl p-5 text-center" style={{ background: "#FBEAF0" }}>
            ...
            <p className="text-xs leading-relaxed" style={{ color: "#72243E" }}>{catContent(activeCategory)}</p>
          </div>
          <div className="grid grid-cols-2 gap-3 px-4 py-5">
            ...
            {catProducts.length === 0 && (
              <p className="col-span-2 text-center text-xs py-8" style={{ color: "#993556" }}>No products in this category yet.</p>
            )}
          </div>
        </div>
      </PageShell>
    ...
    return (
      <PageShell onBack={() => setPage("category")} title={p.category}>
        <div className="max-w-md mx-auto px-4 py-4">
          <div className="relative rounded-2xl overflow-hidden mb-4" style={{ border: "1px solid #F4C0D1" }}>
          ...
          <h2 className="font-medium mb-1" style={{ fontSize: 17, color: "#4B1528" }}>{p.name}</h2>
          <p className="text-sm mb-2" style={{ color: "#72243E" }}>{p.description}</p>
          <div className="font-medium mb-4" style={{ fontSize: 18, color: "#D4537E" }}>₱{p.price}</div>
          <button onClick={() => setOrderModal(true)} className="w-full py-3 rounded-full font-medium text-sm mb-6" style={{ background: "#D4537E", color: "#fff" }}>
            Order now
          </button>
