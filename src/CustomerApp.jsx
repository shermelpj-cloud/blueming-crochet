...
// Consistent full-width layout: max content width with a comfortable side gutter
...
// Live, auto-advancing review carousel
...
// Reusable banner: shows an uploaded image edge-to-edge if set, else a colored fallback
function PageBanner({ imageUrl, emoji, title, desc, showTitle = true }) {
  return (
    <div className="w-full mb-2">
      ...
      <div className={`${WRAP} pt-5 pb-2 text-center`}>
        <div className="max-w-2xl mx-auto">
          {showTitle && <h2 className="font-medium mb-2" style={{ fontSize: 18, color: "#4B1528" }}>{title}</h2>}
          <p className="text-xs md:text-sm leading-relaxed" style={{ color: "#72243E" }}>{desc}</p>
        </div>
      </div>
    </div>
  );
...
function PageShell({ children, onBack, title }) {
  return (
    <div className="min-h-screen" style={{ background: "#FFF9FB" }}>
      {onBack && (
        <div className="sticky top-0 z-20 py-3" style={{ background: "#FFF9FB", borderBottom: "1px solid #F4C0D1" }}>
...
export default function BluemingCrochet() {
  ...
  const catRow = (cat) => content[`category_${cat}`] || {};
  ...
  if (page === "home") {
    return (
      <div style={{ background: "#FFF9FB", minHeight: "100vh" }}>
        ...
        <div className={`${WRAP} py-8 md:py-12 text-center`}>
  ...
  if (page === "category") {
    ...
    const row = catRow(activeCategory);
    return (
      <PageShell onBack={() => setPage("shop")} title={activeCategory}>
        <PageBanner imageUrl={row.image_url} emoji={ICONS[activeCategory]} title={activeCategory} desc={row.description} />
